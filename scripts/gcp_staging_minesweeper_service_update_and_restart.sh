#!/bin/bash
# Staging deploy script for GCP instance (pgl-minesweeper-web).
# Run as: ajaxchess_gmail_com  (cron or manual)

# --- Configuration ---
STATE_DIR="/home/ajaxchess_gmail_com/deploy_state"
REPO_DIR="/home/ajaxchess_gmail_com/git/staging.minesweeper.org"
PROD_REPO_DIR="/home/ajaxchess_gmail_com/git/minesweeper.org"
VENV_DIR="$REPO_DIR/venv"
SERVICE_NAME="minesweeper-staging"
source "$REPO_DIR/.env"

if [ "$(id -u)" -eq 0 ]; then
    echo "Error: This script must not be run as root. Run as the 'ajaxchess_gmail_com' user."
    exit 1
fi
