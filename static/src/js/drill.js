 ...
 *
 * Do not edit static/js/drill.js directly — edit this file.
 */
! function() {
    "use strict";
    var e = document.querySelector(".dr-page"),
        t = e ? parseInt(e.getAttribute("data-drill-id"), 10) : null,
        n = {
            drill: null,
            currentIndex: 0,
            boardClickAt: null,
    ...
    function d(e, t) {
        ...
        }, t || {})).then(async function(e) {
            if (!e.ok) {
                var t = "HTTP " + e.status;
                try {
                    var n = await e.json();
                    n && n.detail && (t = n.detail)
                } catch (e) {}
                throw new Error(t)
            }
            return e.json()
    ...
    function s() {
        var e = n.currentIndex,
            ...
            o = t.boards[e];
        if (o) {
            v("dr-progress-label", e + 1 + " / " + t.num_boards), v("dr-accuracy-label", (t.attempts || []).filter(function(e) {
            ...
            (function(e) {
                    ...
                    if (t) {
                        ...
                        for (var n = new Set((e.revealed || []).map(function(e) {
                                ...
                                return e[0] + "," + e[1]
                            })), d = new Map((e.numbers || []).map(function(e) {
                                return [e[0] + "," + e[1], e[2]]
                            })), i = e.drill_type || r, c = !!l[i], s = !!a[i], m = 0; m < e.height; m++)
                            for (var h = 0; h < e.width; h++) {
                                var f = document.createElement("div");
                                f.className = "dr-cell", f.setAttribute("role", "gridcell"), f.dataset.row = m, f.dataset.col = h;
                                var p = m + "," + h;
                                if (o.has(p)) f.classList.add("dr-cell--flagged"), f.textContent = "🚩";
                                else if (n.has(p)) {
                                    f.classList.add("dr-cell--revealed");
                                    var g = d.get(p);
                                    "number" == typeof g && (f.classList.add("dr-cell--n" + g), f.textContent = String(g)), c && "number" == typeof g && (f.classList.add("dr-cell--chord-target"), f.addEventListener("click", u))
                                } else s && f.addEventListener("click", u);
                                t.appendChild(f)
                            }
                    }
            })(o);
    ...
    async function u(e) {
        ...
        if (l && !l.classList.contains("dr-cell--flagged")) {
            ...
            var d, s = parseInt(l.dataset.row, 10),
                u = parseInt(l.dataset.col, 10),
                m = n.boardClickAt || Date.now(),
                h = window.performance && performance.now ? performance.now() : Date.now(),
                f = Math.max(0, Math.round(h - m));
            try {
                d = await i("/api/drills/" + t + "/submit", {
                    board_index: n.currentIndex,
                    chosen_row: s,
                    chosen_col: u,
                    decision_ms: f
                })
            } catch (e) {
                return void _("Submit failed: " + (e && e.message ? e.message : "unknown"))
            }
            var b = d.result;
            if (b.is_correct) l.classList.add("dr-cell--correct"), p(a, b.revealed_cells, "dr-cell--reveal-self");
            else {
                l.classList.add("dr-cell--wrong");
                var v = a ? a.querySelector('[data-row="' + b.optimal_row + '"][data-col="' + b.optimal_col + '"]') : null;
                v && v.classList.add("dr-cell--optimal"), p(a, b.revealed_cells, "dr-cell--reveal-self"), p(a, b.optimal_revealed_cells, "dr-cell--reveal-optimal")
            }! function(e, t) {
                var l = document.getElementById("dr-feedback"),
                    a = document.getElementById("dr-feedback-icon"),
                    d = document.getElementById("dr-feedback-title"),
                    i = document.getElementById("dr-feedback-body"),
                    s = document.getElementById("dr-next-btn");
                if (l && a && d && i && s) {
            ...
            }(b, d), n.drill.attempts || (n.drill.attempts = []), n.drill.attempts.push({
                board_index: n.currentIndex,
                chosen_row: s,
                chosen_col: u,
                decision_ms: f,
    ...
    function m() {
        b("dr-feedback");
        var e = n.lastResponse,
            t = n.drill && n.drill.num_boards || 0;
        if (e && e.completed || n.currentIndex >= t - 1) return n.completed = !0, void h(e && e.summary || null);
        n.currentIndex += 1, s()
    }
    ...
    function h(e) {
        ...
        if (b("dr-runner"), g("dr-results"), !e) {
            var t = n.drill || {},
                ...
                a = t.num_boards || r.length || 0,
                o = r.map(function(e) {
                    return e.decision_ms
                }).filter(function(e) {
                    return e > 0
                }),
                d = o.length ? Math.round(o.reduce(function(e, t) {
                    return e + t
                }, 0) / o.length) : 0;
            e = {
                num_correct: l,
                num_total: a,
                accuracy_pct: a ? Math.round(1e3 * l / a) / 10 : 0,
                avg_decision_ms: d,
                mastery_contribution: a ? l / a : 0,
                counted_toward_mastery: !1
            }
        }
        v("dr-stat-accuracy", e.num_correct + " / " + e.num_total + "  (" + e.accuracy_pct.toFixed(0) + "%)"), v("dr-stat-avg", (e.avg_decision_ms / 1e3).toFixed(1) + "s"), v("dr-stat-mastery", e.mastery_contribution.toFixed(2));
        var i = document.getElementById("dr-results-icon"),
            c = document.getElementById("dr-results-title"),
    ...
    function p(e, t, n) {
        e && t && t.length && t.forEach(function(t) {
            var r = t[0],
                l = t[1],
                a = t[2],
                o = e.querySelector('[data-row="' + r + '"][data-col="' + l + '"]');
            o && (o.classList.contains("dr-cell--revealed") || o.classList.contains("dr-cell--correct") || o.classList.contains("dr-cell--wrong") || o.classList.contains("dr-cell--optimal") || (o.classList.add("dr-cell--reveal"), n && o.classList.add(n), "number" == typeof a && a > 0 ? (o.classList.add("dr-cell--n" + a), o.textContent = String(a)) : o.textContent = ""))
        })
    }
    ...
    function _(e) {
        b("dr-loading"), b("dr-runner"), g("dr-error"), v("dr-error-detail", e || "")
    }
    document.addEventListener("DOMContentLoaded", async function() {
        if (t && !Number.isNaN(t)) try {
            ...
            var l = document.getElementById("dr-next-btn"),
                a = document.getElementById("dr-again-btn");
            l && l.addEventListener("click", m), a && a.addEventListener("click", f), s()
        } catch (e) {
            _(e && e.message ? e.message : "Unknown error")
        } else _("Missing drill id in page URL.")
    })
}();