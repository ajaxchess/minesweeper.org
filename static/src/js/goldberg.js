 ...
 *
 * Uses pre-baked geometry from goldberg_prebaked.js (generated by
 * scripts/bake_goldberg.py using antiprism_python) when available.
 * Falls back to runtime computation for custom (a,b) configs not in the
 * prebaked set; the runtime path is correct for Class I (b=0) only.
 *
 * @returns {{ faces, adj, T, F, pentagons }}
 */
function goldberg(a, b) {
    const key = `${a},${b}`;
    if (typeof GOLDBERG_PREBAKED !== 'undefined' && GOLDBERG_PREBAKED[key]) {
        const pre = GOLDBERG_PREBAKED[key];
        const T = a * a + a * b + b * b;
        const pentagons = new Set(
            pre.faces.map((f, i) => (f.isPentagon ? i : -1)).filter(i => i >= 0)
        );
        return { faces: pre.faces, adj: pre.adj, T, F: pre.faces.length, pentagons };
    }
