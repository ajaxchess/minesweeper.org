...
function updateCatCounter() {
    ...
    const sep = document.querySelector('.stat-sep');
    if (sep) sep.textContent = '/' + G.n;
}
...
function checkWin() {
    ...
    // NOTE: deliberately no check against G.catCols. A board can have more than
    // one valid arrangement, and requiring the player's cats to sit in the
    // generator's own columns rejected correct solutions (ba3450cd).
