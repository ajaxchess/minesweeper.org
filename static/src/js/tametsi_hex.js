...
const THEX_PUZZLES = {
  ...
  8: {
    R: 4,
    mines: new Set(['2,-3','1,-2','3,-2','0,-1','1,1','-2,3','-1,3','3,-3','2,-2','1,-1','-2,2','-2,-1','-3,0','2,1']),
    cellColors: {
      '0,-3':'green', '-2,-1':'green', '3,-1':'green', '-3,0':'green',
      '2,1':'green', '1,2':'green', '0,3':'green',
      '3,-3':'yellow', '2,-2':'yellow', '1,-1':'yellow', '0,0':'yellow',
      '-1,1':'yellow', '-2,2':'yellow', '-3,3':'yellow',
    },
    startRevealed: { '-2,1':1, '-3,3':2, '0,3':1 },
    tutorialText: '<strong>Puzzle 8 — Yellow &amp; Green (R=4, 14 mines):</strong> Two new colour groups: <span style="color:#f0c040"><strong>yellow</strong></span> (4 mines) and <span style="color:#4caf50"><strong>green</strong></span> (3 mines). The counter bar shows <span style="color:#f0c040"><strong>🟡&thinsp;4:4</strong></span> and <span style="color:#4caf50"><strong>🟢&thinsp;3:3</strong></span> in <em>total:remaining</em> format — flag a mine in a group and its remaining count drops. Numbers on every cell count <strong>all</strong> adjacent mines regardless of colour.',
  },
};
  ...
  for (const [q,r] of cells) {
    ...
    if (isPreRevealed) {
      ...
      if (val > 0) {
        ...
        // -2 marks a revealed cell whose adjacent-mine count is deliberately hidden
        txt.textContent = '?';
...
function thexUpdateCounter() {
  ...
  if (thexState.colorMines) {
    ...
    const CLS    = {red:'thex-ccount-red', blue:'thex-ccount-blue', yellow:'thex-ccount-yellow', green:'thex-ccount-green'};
