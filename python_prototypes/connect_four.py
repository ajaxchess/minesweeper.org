...
def place_piece():
  if (board[5][move_position])==0:
    board[5][move_position] = move_piece
  elif (board[4][move_position])==0:
    board[4][move_position] = move_piece
  elif (board[3][move_position])==0:
    board[3][move_position] = move_piece
  elif (board[2][move_position])==0:
    board[2][move_position] = move_piece
  elif (board[1][move_position])==0:
    board[1][move_position] = move_piece
  elif (board[0][move_position])==0:
    board[0][move_position] = move_piece
  else:
    print("Column full")
...
# Proper 2D board initialization to avoid row aliasing
board = [[0 for _ in range(columns)] for _ in range(rows)]
print_board(board)
...
play_game()
'''
print ("enter an x or o and a column Number")
stdin_move = int(input("It is " + turn + "'s turn. Please select a column. Your options are " + str(available)))

#fields = stdin_move.split()
move_piece = "x"
move_position = stdin_move
...
#print ("  +---------------+")
#for i in range(rows):
#  print (i,"|",board[i][0],board[i][1],board[i][2],board[i][3],board[i][4],board[i][5],board[i][6],"|")
