"""
tests/test_globesweeper.py — Globesweeper score submission and leaderboard (F55).
"""
...
class TestGlobesweeperPages:
    def test_beginner_page_ok(self, client):
        r = client.get("/globesweeper")
        assert r.status_code == 200

    def test_intermediate_page_ok(self, client):
        r = client.get("/globesweeper/intermediate")
        assert r.status_code == 200

    def test_expert_page_ok(self, client):
        r = client.get("/globesweeper/expert")
        assert r.status_code == 200

    def test_custom_page_ok(self, client):
        r = client.get("/globesweeper/custom")
        assert r.status_code == 200

    def test_leaderboard_page_ok(self, client):
        r = client.get("/globesweeper/leaderboard")
        assert r.status_code == 200


class TestGlobesweeperScoreSubmit:
    def test_beginner_submit_returns_201(self, client):
        r = client.post("/api/globesweeper-scores", json=BEGINNER_SCORE, headers=XHR)
        assert r.status_code == 201

    def test_intermediate_submit_returns_201(self, client):
        r = client.post("/api/globesweeper-scores", json=INTERMEDIATE_SCORE, headers=XHR)
        assert r.status_code == 201

    def test_expert_submit_returns_201(self, client):
        r = client.post("/api/globesweeper-scores", json=EXPERT_SCORE, headers=XHR)
        assert r.status_code == 201
    ...
    def test_submit_with_board_hash(self, client):
        ...
        r = client.post("/api/globesweeper-scores", json=score, headers=XHR)
        assert r.status_code == 201
...
class TestGlobesweeperLeaderboard:
    def test_empty_leaderboard(self, client):
        r = client.get("/api/globesweeper-scores/beginner")
        assert r.status_code == 200
        assert r.json() == []

    def test_score_appears_in_leaderboard(self, client):
        client.post("/api/globesweeper-scores", json=BEGINNER_SCORE, headers=XHR)
        r = client.get("/api/globesweeper-scores/beginner")
        assert r.status_code == 200
    ...
    def test_leaderboard_sorted_by_time_asc(self, client):
        ...
        client.post("/api/globesweeper-scores", json=slow, headers=XHR)
        client.post("/api/globesweeper-scores", json=fast, headers=XHR)
        scores = client.get("/api/globesweeper-scores/beginner").json()
        names = [s["name"] for s in scores]
        assert names.index("Fast") < names.index("Slow")

    def test_modes_are_independent(self, client):
        client.post("/api/globesweeper-scores", json=BEGINNER_SCORE,      headers=XHR)
        client.post("/api/globesweeper-scores", json=INTERMEDIATE_SCORE,  headers=XHR)
        beg_names  = {s["name"] for s in client.get("/api/globesweeper-scores/beginner").json()}
        int_names  = {s["name"] for s in client.get("/api/globesweeper-scores/intermediate").json()}
    ...
    def test_invalid_mode_returns_400(self, client):
        r = client.get("/api/globesweeper-scores/sphere")
...
class TestGlobesweeperValidation:
    def test_missing_name_rejected(self, client):
        ...
        r = client.post("/api/globesweeper-scores", json=bad, headers=XHR)
    ...
    def test_no_csrf_header_rejected(self, client):
        r = client.post("/api/globesweeper-scores", json=BEGINNER_SCORE)
