...
(function () {
  ...
  function bindProgressModal() {
    ...
    document.addEventListener('click', function (ev) {
      ...
      const drillBtn = ev.target.closest('[data-action="start-drill"]');
      if (drillBtn) {
        ev.preventDefault();
        const level = parseInt(drillBtn.dataset.level, 10);
  ...
  async function startDrill(level, btnEl) {
    ...
    try {
      const resp = await fetch('/api/drills/start', {
        method: 'POST',
        credentials: 'same-origin',
        headers: {
      ...
      window.location.href = '/drill/' + data.drill_id;
    } catch (err) {
      if (btnEl) {
        btnEl.disabled = false;
        btnEl.textContent = 'Couldn’t start — retry';
      }
      // eslint-disable-next-line no-console
      console.error('startDrill failed', err);
    }
  }
